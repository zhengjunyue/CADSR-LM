...
nj=14
train_set=train
test_sets="test test_head test_head_word test_head_sentence test_array test_array_word test_array_sentence"
...
if [ $stage -le 13 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 42 --cmd "$train_cmd" ${lores_train_data_dir} \
...
if [ $stage -le 14 ]; then
  ...
  steps/nnet3/chain/build_tree.sh \
    ...
    --cmd "$train_cmd" 1260 ${lores_train_data_dir} \
...
if [ $stage -le 15 ]; then
  ...
  fixed-affine-layer name=lda delay=5 input=Append(-2,-1,0,1,2) affine-transform-file=$dir/configs/lda.mat
...
if [ $stage -le 17 ]; then
  ...
  utils/mkgraph.sh \
    ...
    $tree_dir $tree_dir/graph_test || exit 1;
    ...
    (
      ...
      nspk=$(wc -l <data/${data}_hires/spk2utt)
        ...
        steps/nnet3/decode_looped.sh \
          ...
          $tree_dir/graph data/${data}_hires ${dir}/decode_looped_${data_affix} || exit 1
  
      steps/lmrescore.sh \
        --self-loop-scale 1.0 \
        --cmd "$decode_cmd" data/lang_test \
        data/${data}_hires ${dir}/decode_looped_${data_affix} || exit 1
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
        data/lang_test \
       data/${data}_hires ${dir}/decode_looped_${data_affix}{,_fg} || exit 1
    ) || touch $dir/.error &
  done
  wait
  [ -f $dir/.error ] && echo "$0: there was a problem while decoding" && exit 1
