...
local/torgo_prepare_lm_tgwsj.sh --order ${lm_order} || exit 1
#local/train_lms_srilm.sh data/lang_nosp data/train/text data/test/text data/lang_opt lm.gz || exit 1
...
echo "--- Monophone decoding"    
  utils/mkgraph.sh --mono data/lang_test exp/mono exp/mono/graph_test || exit 1
...
# decode tri1
  ...
  utils/mkgraph.sh data/lang_test exp/tri1 exp/tri1/graph_test || exit 1;
  for x in "${tests[@]}"; do
  steps/decode.sh --config conf/decode.config --nj $njobsT --cmd "$decode_cmd" \
...
# decode tri2a
  ...
  utils/mkgraph.sh data/lang_test exp/tri2a exp/tri2a/graph_test
  for x in "${tests[@]}"; do
  steps/decode.sh --config conf/decode.config --nj $njobsT --cmd "$decode_cmd" \
...
# decode tri2b
  ...
  utils/mkgraph.sh data/lang_test exp/tri2b exp/tri2b/graph_test
  for x in "${tests[@]}"; do
  steps/decode.sh --config conf/decode.config --nj $njobsT --cmd "$decode_cmd" \
...
steps/train_mmi.sh --cmd "$train_cmd" data/train data/lang exp/tri2b_ali exp/tri2b_denlats exp/tri2b_mmi || exit 1;
...
steps/train_sat.sh --cmd "$train_cmd" $leaves $gaussians data/train data/lang exp/tri2b_ali exp/tri3b || exit 1;


  utils/mkgraph.sh data/lang_test exp/tri3b exp/tri3b/graph_test || exit 1;
  for x in "${tests[@]}"; do
...
steps/train_diag_ubm.sh --silence-weight 0.5 --nj $njobs --cmd "$train_cmd" \
  250 data/train data/lang exp/tri3b_ali exp/dubm3b  ##need to be edited 
...
# decode using the tri4b model
  ...
  utils/mkgraph.sh data/lang_test exp/tri4b exp/tri4b/graph_test || exit 1;
  for x in "${tests[@]}"; do
