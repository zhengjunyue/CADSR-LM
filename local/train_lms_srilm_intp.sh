...
if [ ! -z ${LIBLBFGS} ]; then
  ...
  echo "Maxent 2grams"
  echo "-------------------"
  sed 's/'${oov_symbol}'/!SIL/g' $tgtdir/train.txt | \
    ngram-count -lm - -order 2 -text - -vocab $tgtdir/vocab -unk -sort -maxent -maxent-convert-to-arpa|\
    sed 's/!SIL/'${oov_symbol}'/g' | gzip -c > $tgtdir/2gram.me.gz || exit 1
  ...
  sed 's/'${oov_symbol}'/!SIL/g' $tgtdir/train.txt | \
    ...
    sed 's/!SIL/'${oov_symbol}'/g' | gzip -c > $tgtdir/3gram.me.gz || exit 1
  ...
  sed 's/'${oov_symbol}'/!SIL/g' $tgtdir/train.txt | \
    ...
    sed 's/!SIL/'${oov_symbol}'/g' | gzip -c > $tgtdir/4gram.me.gz || exit 1
