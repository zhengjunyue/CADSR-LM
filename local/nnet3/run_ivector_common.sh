...
test_sets="test test_word test_sentence test_head test_head_word test_head_sentence test_array test_array_word test_array_sentence"
...
if [ $stage -le 2 ]; then
  ...
  for datadir in ${test_sets}; do
    steps/make_mfcc.sh --nj 1 --mfcc_config conf/mfcc_hires.conf \
  ...
  steps/make_mfcc.sh --nj 14 --mfcc_config conf/mfcc_hires.conf \
    --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
  steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
  utils/fix_data_dir.sh data/${datadir}_hires || exit 1;
  done
  ...
  utils/data/subset_data_dir.sh data/${train_set}_sp_hires \
      $num_utts ${temp_data_root}/${train_set}_sp_hires
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 14 \
    --num-frames 150000 \
    --num-threads $num_threads_ubm \
    ${temp_data_root}/${train_set}_sp_hires_subset 256 \
